...
/**
 * @title NNS Registry
 * @dev The NNS registry contract manages domain ownership and resolution for .nex domains
 */
contract NNSRegistry is Ownable, ERC165 {
    struct Record {
        address owner;
        address resolver;
        uint64 ttl;
    ...
    modifier live(bytes32 node) {
        require(records[node].owner != address(0), "Domain does not exist");
        _;
    }
     ...
     * @param resolver The address of the resolver.
     */
    function setResolver(bytes32 node, address resolver) public virtual authorised(node) {
     ...
     *      all of `msg.sender`'s NNS records.
